...
void print_elf64_header(const Elf64_Ehdr* ehdr) {
	...
	print_string("Magic: ");
	for (int i = 0; i < 4; i++) {
		print_string(itoa_unsigned(ehdr->e_ident[i], buf, INTEGER_BASE_HEX));
		print_string(" ");
	}
	print_string("\n");
	...
	print_string("Class: ");
	print_string(itoa_unsigned(ehdr->e_ident[EI_CLASS], buf, INTEGER_BASE_DECIMAL));
	print_string("\n");

	// 3. Object Type and Machine Architecture
	print_string("Type: 0x");
	print_string(itoa_unsigned(ehdr->e_type, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("Machine: 0x");
	print_string(itoa_unsigned(ehdr->e_machine, buf, INTEGER_BASE_HEX));
	print_string("\n");

	// 4. Memory Addresses and Offsets
	print_string("Entry Point: 0x");
	print_string(itoa_unsigned(ehdr->e_entry, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("Program Header Offset: 0x");
	print_string(itoa_unsigned(ehdr->e_phoff, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("Section Header Offset: 0x");
	print_string(itoa_unsigned(ehdr->e_shoff, buf, INTEGER_BASE_HEX));
	print_string("\n");

	// 5. Sizes and Counts (Usually easier to read in Decimal)
	print_string("ELF Header Size: ");
	print_string(itoa_unsigned(ehdr->e_ehsize, buf, INTEGER_BASE_DECIMAL));
	print_string(" bytes\n");

	print_string("Program Header Entry Size: ");
	print_string(itoa_unsigned(ehdr->e_phentsize, buf, INTEGER_BASE_DECIMAL));
	print_string(" bytes\n");

	print_string("Program Header Count: ");
	print_string(itoa_unsigned(ehdr->e_phnum, buf, INTEGER_BASE_DECIMAL));
	print_string("\n");

	print_string("Section Header Entry Size: ");
	print_string(itoa_unsigned(ehdr->e_shentsize, buf, INTEGER_BASE_DECIMAL));
	print_string(" bytes\n");

	print_string("Section Header Count: ");
	print_string(itoa_unsigned(ehdr->e_shnum, buf, INTEGER_BASE_DECIMAL));
	print_string("\n");
...
void print_elf64_program_header(Elf64_Phdr* phdr, uint32_t index) {
	...
	print_string(itoa_unsigned(index, buf, INTEGER_BASE_DECIMAL));
	print_string("] ---\n");

	// 1. Type (1 = PT_LOAD, the only one we care about for booting)
	print_string("Type: 0x");
	print_string(itoa_unsigned(phdr->p_type, buf, INTEGER_BASE_HEX));
	...
	print_string(itoa_unsigned(phdr->p_vaddr, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("LMA (Load Addr): 0x");
	print_string(itoa_unsigned(phdr->p_paddr, buf, INTEGER_BASE_HEX));
	print_string("\n");

	// 3. File Location and Sizes
	print_string("Offset in File: 0x");
	print_string(itoa_unsigned(phdr->p_offset, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("Size in File: ");
	print_string(itoa_unsigned(phdr->p_filesz, buf, INTEGER_BASE_DECIMAL));
	print_string(" bytes\n");

	print_string("Size in RAM:  ");
	print_string(itoa_unsigned(phdr->p_memsz, buf, INTEGER_BASE_DECIMAL));
	print_string(" bytes\n");

	// 4. Flags and Alignment
	print_string("Flags: 0x");
	print_string(itoa_unsigned(phdr->p_flags, buf, INTEGER_BASE_HEX));
	print_string("\n");

	print_string("Alignment: 0x");
	print_string(itoa_unsigned(phdr->p_align, buf, INTEGER_BASE_HEX));
	print_string("\n------------------------\n");
}
