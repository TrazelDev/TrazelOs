#pragma once
// Was taken from another project of mine: https://github.com/TrazelDev/fat12-parser
...
static inline bool is_final_directory_entry(struct fat12_directory_entry* entry) {
	return entry->file_name[0] == FINAL_ENTRY;
}
static inline bool is_volume_label_entry(struct fat12_directory_entry* entry) {
	return entry->attributes & VOLUME_LABEL_ATTRIBUTE;
}
static inline bool is_deleted_entry(struct fat12_directory_entry* entry) {
	return ((uint8_t)entry->file_name[0] == DELETED_ENTRY);
}
static inline bool is_directory_entry_directory(const struct fat12_directory_entry* entry) {
...
/** Reads bytes from a device from an offset and loads into a preallocated buffer.
 ...
 * @param[in] devicePath Path to loop device that will be read
 */
void pread_device(uint8_t* buffer, uint64_t read_bytes, int64_t offset);

/** Loads struct FAT12Header with information from a loop device.
...
/** Loads struct FAT12Info from struct FAT12Header.
 * @param[out] fat12Info Pointer to the allocated structure to load information to.
 * @param[in] fat12Header A struct FAT12Header structure with correct information of some loop
 * device.
...
/** Gets the root folder file names in an array from the device using struct FAT12Info
 *
 ...
 * @param[in] fat12Info A struct FAT12Info that was created from the device.
 * @param[in] devicePath.
 *
 ...
 * @param[in] dirEntries Pointer to an array of struct FAT12DirectoryEntry structures.
...
/** Extracts fat12 directory entries of specific directory from the device provided.
 * This entries include:
 * directories, files, deleted entries, long file name entries and volume label entries.
 *
 * @param[out] dirs Pointer to an array of struct FAT12DirectoryEntry that will be allocated
 * internally.
 ...
 * filter.
 *
 * @param[in,out] dirEntries Pointer to an array of struct FAT12DirectoryEntry that already contains
 * entries, on input holds pointer to an unfiltered array of struct FAT12DirectoryEntries, with
 * entriesCount elements, while on output holds pointer to a filtered array of
 * struct FAT12DirectoryEntries.
 ...
 * described by fat12Info located at devicePath. The file data is returned via fileContent
 ...
 * @param[in] fat12Info
 * @param[in] devicePath Path to loop device.
 *
...
/** Loads the FAT (File Allocation Table) from a FAT12 device.
 * The function reads the first FAT in the FAT section of the filesystem located in device.
 * @param[in] fat12Info
 * @param[in] devicePath Path to filesystem loop device.
 ...
 * loop device devicePath points to.
 *
...
/** Extracts FAT12 root directory entries from the device provided.
