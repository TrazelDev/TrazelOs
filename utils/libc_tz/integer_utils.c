...
/*
 * takes a number and returns the corresponding ascii value:
 * 0-9: ascii of 0-9
 * 10-15: ascii of A-F
 */
...
char* itoa_signed(int64_t value, char* buffer, enum base base) {
	// Making place for the negative sign cause part of conversion function is doing a string
	// reverse
	...
	itoa_unsigned((uint64_t)value, buffer, base);
	if (negative_flag) {
		buffer--;
		buffer[0] = '-';
	}
